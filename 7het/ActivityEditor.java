...
public class ActivityEditor extends javafx.application.Application {
    ...
    public void start(javafx.stage.Stage stage) {
        ...
        if(cmdParams.containsKey("color")){
//            if(cmdParams.get("color").contentEquals( "rose"))
//                 scene.getStylesheets().add(getClass().getResource("rose.css").toExternalForm());
//            else if(cmdParams.get("color").contentEquals( "green"))
//                scene.getStylesheets().add(getClass().getResource("green.css").toExternalForm()); //hard codeolt, csak a 3 stíluslap használható
//            else if(cmdParams.get("color").contentEquals( "gray"))
//               scene.getStylesheets().add(getClass().getResource("gray.css").toExternalForm()); 
            String color = "";
            color = cmdParams.get("color");
            color += ".css";
            try{
                scene.getStylesheets().add(getClass().getResource(color).toExternalForm());      //EDIT: hozzáadhatunk stíluslapokat, és használható
            }                                                                                    // color=<*.css> argumentummal
            catch (Exception e){
                System.err.println(" Can't find "+color+". The program runs with default colors.");
            }
        }                                                                                           
